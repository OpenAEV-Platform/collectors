"""HTTPLogwatcher Trace Service Provider."""

import logging
from datetime import UTC, datetime
from typing import Any

from src.models.expectation import ExpectationResult, ExpectationTrace
...
LOG_PREFIX = "[HTTPLogwatcherTraceService]"


class TraceService:
    """HTTPLogwatcher-specific trace service provider.
    ...
    def __init__(self, config: ConfigLoader | None = None) -> None:
        ...
        self.config = config
        self.logger.debug(f"{LOG_PREFIX} HTTPLogwatcher trace service initialized")
    ...
    def create_traces_from_results(
        ...
        Raises:
            HTTPLogwatcherValidationError: If inputs are invalid.
            HTTPLogwatcherDataConversionError: If trace creation fails.

        """
        if not collector_id:
            raise HTTPLogwatcherValidationError("collector_id cannot be empty")

        if not isinstance(results, list):
            raise HTTPLogwatcherValidationError("results must be a list")
        ...
        except HTTPLogwatcherDataConversionError:
            raise
        except Exception as e:
    ...
    def _create_expectation_trace(
        ...
        """
        if not expectation_id:
            raise HTTPLogwatcherValidationError("expectation_id cannot be empty")
        ...
        try:
            ...
            alert_name = matching_data.get("alert_name", "HTTPLogwatcher Alert")
        ...
        except HTTPLogwatcherValidationError:
            raise
        except Exception as e:
            raise HTTPLogwatcherDataConversionError(
    ...
    def get_service_info(self) -> dict[str, Any]:
        ...
        """
        info = {
            "service_type": "httplogwatcher_trace",
            "supported_result_types": ["HTTPLogwatcher processing results"],
            "creates_detection_traces": True,
            "creates_prevention_traces": True,
            "description": "Creates traces from HTTPLogwatcher expectation processing results using trace builder URLs",
        }
