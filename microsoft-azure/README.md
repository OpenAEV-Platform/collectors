...
- Copy `microsoft_azure/config.yml.sample` to `microsoft_azure/config.yml`
- Update configuration values or set environment variables

3. **Run the Collector**:
   ```bash
   # Using Poetry
   poetry run python -m microsoft_azure
   
   # Or 
   poetry run MicrosoftAzureCollector

   # Or direct execution after installing the project and activating the virtual environment:
   MicrosoftAzureCollector
   ```
