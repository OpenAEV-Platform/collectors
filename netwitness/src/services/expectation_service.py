"""NetWitness Expectation Service Provider.

This module contains all the NetWitness-specific logic for handling expectations.
...
LOG_PREFIX = "[NetWitnessExpectationService]"


class NetWitnessExpectationService:
    """NetWitness-specific service provider for expectation handling.

    This class contains all the business logic specific to NetWitness:
    - Which signature types to support (only IPV4/6 addresses)
    - How to fetch data from NetWitness
    ...
    def __init__(self, config: ConfigLoader | None = None) -> None:
        """Initialize the NetWitness service provider.
        ...
        Raises:
            ...
            NetWitnessConfigurationError: If service components initialization fails.
        ...
        try:
            self.logger.debug(
                f"{LOG_PREFIX} Initializing NetWitness service components..."
            )
            ...
            raise NetWitnessConfigurationError(
                f"Failed to initialize NetWitness service components: {e}"
            ) from e

        if (
            hasattr(config, "netwitness")
            and hasattr(config.netwitness, "time_window")
            and config.netwitness.time_window
        ):
            self.time_window = config.netwitness.time_window
            self.logger.debug(
        ...
        if hasattr(config, "netwitness"):
            self.max_retry = getattr(config.netwitness, "max_retry", 3)
            self.offset = getattr(
                config.netwitness, "offset", timedelta(seconds=30)
            ).total_seconds()
            self.logger.debug(
    ...
    def handle_batch_expectations(
        ...
        Raises:
            NetWitnessExpectationError: If batch processing fails.
        ...
        try:
            ...
            for i, expectation in enumerate(expectations, 1):
                ...
                try:
                ...
                except NetWitnessServiceError as e:
                    self.logger.warning(
                        f"{LOG_PREFIX} NetWitness service error for expectation {expectation_id}: {e}"
                    )
                    ...
                    result = self._create_error_result_object(
                        NetWitnessExpectationError(f"Unexpected error: {e}"),
                        expectation,
                    )
            ...
            raise NetWitnessExpectationError(
    ...
    def process_expectation(
        ...
        Args:
            expectation: The expectation to process (Detection only for NetWitness).
        ...
        Raises:
            NetWitnessExpectationError: If expectation type is unsupported.
        ...
        if isinstance(expectation, DetectionExpectation):
            ...
            self.logger.warning(
                f"{LOG_PREFIX} NetWitness service warning for expectation {expectation_id}: NetWitness only supports DetectionExpectations, not PreventionExpectations, marking them as invalid"
            )
            return ExpectationResult(
                expectation_id=expectation_id,
                is_valid=False,
                expectation=expectation,
                error_message="NetWitness only supports DetectionExpectations, not PreventionExpectations",
            )
        else:
            self.logger.error(
    ...
    def handle_prevention_expectation(
        ...
        Since NetWitness only supports detection, this method logs a warning
        ...
        expectation_id = str(expectation.inject_expectation_id)
        self.logger.warning(
            f"{LOG_PREFIX} NetWitness service error for expectation {expectation_id}: NetWitness only supports DetectionExpectations, not PreventionExpectations"
        )
        return ExpectationResult(
            expectation_id=expectation_id,
            is_valid=False,
    ...
    def _handle_expectation(
        ...
        Raises:
            NetWitnessExpectationError: If expectation processing fails.
        ...
        try:
            ...
            self.logger.debug(
                f"{LOG_PREFIX} Fetching NetWitness data for {expectation_type} expectation..."
            )
            netwitness_data = self.client_api.fetch_with_retry(
                search_signatures, expectation_type, self.max_retry, int(self.offset)
            )
            self.logger.debug(
                f"{LOG_PREFIX} Fetched {len(netwitness_data)} data items from NetWitness"
            )

            self.logger.debug(
                f"{LOG_PREFIX} Converting NetWitness data to OAEV format..."
            )
            oaev_data = self.converter.convert_data_to_oaev_data(netwitness_data)
            self.logger.debug(
    ...
    def _extract_signatures(
        ...
        Raises:
            NetWitnessExpectationError: If signature extraction fails.
    ...
    def _match(
        ...
        Raises:
            NetWitnessNoAlertsFoundError: If no data available for matching.
            NetWitnessNoMatchingAlertsError: If no matching alerts found.
            NetWitnessMatchingError: If matching process fails.

        """
        try:
            if not oaev_data:
                self.logger.debug(f"{LOG_PREFIX} No OAEV data available for matching")
                raise NetWitnessNoAlertsFoundError("No data available for matching")
            ...
            for i, data_item in enumerate(oaev_data):
                ...
                if available_signatures:
                    try:
                        ...
                        raise NetWitnessNoMatchingAlertsError() from e
                else:
                    self.logger.debug(
            ...
            raise NetWitnessNoMatchingAlertsError()
            ...
            raise NetWitnessMatchingError() from e
    ...
    def _create_error_result(
        self,
        error: NetWitnessServiceError,
        expectation: DetectionExpectation | None = None,
    ) -> dict[str, Any]:
        """Create an error result dictionary from a NetWitness service error.

        Args:
            error: The NetWitness service error that occurred.
    ...
    def _create_error_result_object(
        ...
        """Create an ExpectationResult object from a NetWitness service error.
    ...
    def get_service_info(self) -> dict[str, Any]:
        ...
        """
        info = {
            "service_name": "NetWitness",
            "supported_signatures": [sig.value for sig in self.SUPPORTED_SIGNATURES],
            "supports_detection": True,
            "supports_prevention": False,
            "description": f"NetWitness expectation validation service ({len(self.SUPPORTED_SIGNATURES)} signature types, detection only)",
        }
