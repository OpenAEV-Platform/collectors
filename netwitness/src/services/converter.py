"""NetWitness Data Converter.

This module provides conversion functionality for NetWitness data types.
...
from .models import NetWitnessAlert
from .utils.parent_process_parser import ParentProcessParser

LOG_PREFIX = "[NetWitnessConverter]"


class Converter:
    """Converter for NetWitness data to OAEV format."""
    ...
    def __init__(self) -> None:
        ...
        self.logger.debug(f"{LOG_PREFIX} NetWitness data converter initialized")

    def convert_data_to_oaev_data(
        self,
        data: NetWitnessAlert | list[NetWitnessAlert] | None,
    ) -> list[dict[str, Any]]:
        ...
        try:
            self.logger.debug(
                f"{LOG_PREFIX} Converting {len(data)} NetWitness alert items to OAEV format"
            )
            oaev_datas = []
            alert_count = 0
            unknown_count = 0
            ...
            for i, item in enumerate(data, 1):
                ...
                try:
                    if self._is_alert_data(item):
                        ...
                        alert_count += 1
                        self.logger.debug(
                            f"{LOG_PREFIX} Converted NetWitness alert item {i}"
                        )
                    else:
                        unknown_count += 1
                        self.logger.warning(
            ...
            self.logger.info(
                f"{LOG_PREFIX} NetWitness to OAEV conversion: processed {len(data)} items -> {len(oaev_datas)} results"
            )
    ...
    def _is_alert_data(self, data: Any) -> bool:
        """Check if data is NetWitness alert data.

        Args:
            data: Data object to check.

        Returns:
            True if data is a NetWitnessAlert instance.

        """
        return isinstance(data, NetWitnessAlert)

    def _alert_data(self, alert_data: NetWitnessAlert) -> dict[str, Any]:
        """Convert NetWitness alert data to OAEV format.

        Args:
            alert_data: NetWitness alert data.

        Returns:
            OAEV formatted data dictionary.

        Raises:
            NetWitnessValidationError: If input type is invalid.
            NetWitnessDataConversionError: If conversion fails.
        ...
        try:
            ...
            if not isinstance(alert_data, NetWitnessAlert):
            ...
            self.logger.debug(
                f"{LOG_PREFIX} Converted NetWitness alert to OAEV with {len(oaev_data)} fields"
            )
            ...
            raise NetWitnessDataConversionError(
                f"Error converting NetWitness alert data to OAEV: {e}"
            ) from e

    def _extract_source_ips(self, alert_data: NetWitnessAlert) -> list[str]:
        """Extract source IP addresses from alert data.

        Args:
            alert_data: NetWitnessAlert object.
    ...
    def _extract_target_ips(self, alert_data: NetWitnessAlert) -> list[str]:
    ...
    def _extract_parent_process_name(self, alert_data: NetWitnessAlert) -> str:
