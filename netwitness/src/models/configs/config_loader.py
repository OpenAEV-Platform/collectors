...
class ConfigLoaderCollector(_ConfigLoaderCollector):
    ...
    for the NetWitness collector instance.
    """

    id: str = Field(
        alias="Collector_ID",
        default="netwitness--0b13e3f7-5c9e-46f5-acc4-33032e9b4921",
...
class ConfigLoader(ConfigBaseSettings):
    ...
    Main configuration class that combines OpenAEV, collector, and NetWitness
    ...
    netwitness: _ConfigLoaderNetWitness = Field(
        default_factory=_ConfigLoaderNetWitness,
        description="NetWitness configurations.",
    )
    ...
    def to_daemon_config(self) -> Configuration:
        ...
        return Configuration(
            config_hints={
                ...
                # NetWitness configuration (flattened)
                "netwitness_base_url": {"data": str(self.netwitness.base_url)},
                "netwitness_token": {
                    "data": (
                        self.netwitness.token.get_secret_value()
                        if self.netwitness.token
                        else None
                    )
                },
                "netwitness_username": {"data": self.netwitness.username},
                "netwitness_password": {
                    "data": (
                        self.netwitness.password.get_secret_value()
                        if self.netwitness.password
                        else None
                    )
                },
                "netwitness_max_results": {"data": self.netwitness.max_results},
                "netwitness_console_url": {"data": self.netwitness.console_url},
                "netwitness_time_window": {"data": self.netwitness.time_window},
                "netwitness_max_retry": {"data": self.netwitness.max_retry},
                "netwitness_offset": {"data": self.netwitness.offset},
                "netwitness_verify_ssl": {"data": self.netwitness.verify_ssl},
            },
