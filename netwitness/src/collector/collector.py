...
from src.services.utils import NetWitnessConfig
...
COLLECTOR_TYPE = "openaev_netwitness"
...
class Collector(CollectorDaemon):  # type: ignore[misc]
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            self.logger.info(  # type: ignore[has-type]
                f"{LOG_PREFIX} NetWitness Collector initialized successfully"
            )
    ...
    def _setup(self) -> None:
        ...
        Initializes NetWitness services, expectation handler, expectation manager,
        ...
        try:
            ...
            self.logger.debug(f"{LOG_PREFIX} Initializing NetWitness services...")

            self.netwitness_service = NetWitnessExpectationService(self.config_instance)

            self.trace_service = NetWitnessTraceService(self.config_instance)

            self.expectation_handler = GenericExpectationHandler(
                self.netwitness_service
            )
            ...
            supported_signatures = self.netwitness_service.get_supported_signatures()
            ...
            service_info = self.netwitness_service.get_service_info()
    ...
    def _process_callback(self) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"{LOG_PREFIX} Processing expectations using NetWitness services"
            )
