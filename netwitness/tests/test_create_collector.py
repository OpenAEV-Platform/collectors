"""Test module for the NetWitness Collector initialization."""
...
def collector_config() -> dict[str, str]:  # type: ignore
    ...
    return {
        ...
        "COLLECTOR_NAME": "NetWitness",
        "NETWITNESS_BASE_URL": "https://fake-netwitness.net:50103/",
        "NETWITNESS_USERNAME": "fake-user",
        "NETWITNESS_PASSWORD": "fake-password",
...
def test_success_create_collector(capfd, collector_config):  # type: ignore
    """Test that the main function initializes and start the NetWitness Collector.
    ...
    """
    # Given I have a valid configuration to start the NetWitness Collector.
...
def test_collector_config_missing_required_values() -> None:
    ...
    configuration values are missing, specifically the NetWitness username.

    """
    # Given configuration with missing required NetWitness username
    data = {
        "OPENAEV_URL": "http://fake-url",
        "OPENAEV_TOKEN": "fake-oaev-token",
        "OPENAEV_TENANT_ID": "deadbeef-dead-beef-dead-beefdeadbeef",
        "COLLECTOR_ID": "fake-collector-id",
        "COLLECTOR_NAME": "NetWitness",
        "NETWITNESS_BASE_URL": "https://fake-netwitness.net:9200/",
        # Missing NETWITNESS_USERNAME - this should cause validation error
        "NETWITNESS_PASSWORD": "fake-password",
        "NETWITNESS_ALERTS_INDEX": ".alerts-security.alerts-*",
        "COLLECTOR_ICON_FILEPATH": "src/img/netwitness-logo.png",
        "COLLECTOR_LOG_LEVEL": "debug",
    }
    mock_env = _given_setup_config(data)
...
def test_collector_config_missing_password() -> None:
    ...
    """
    # Given configuration with missing required NetWitness password
    data = {
        "OPENAEV_URL": "http://fake-url",
        "OPENAEV_TOKEN": "fake-oaev-token",
        "OPENAEV_TENANT_ID": "deadbeef-dead-beef-dead-beefdeadbeef",
        ...
        "NETWITNESS_BASE_URL": "https://fake-netwitness.net:9200/",
        "NETWITNESS_USERNAME": "fake-user",
        # Missing NETWITNESS_PASSWORD - this should cause validation error
        "NETWITNESS_ALERTS_INDEX": ".alerts-security.alerts-*",
        "COLLECTOR_ICON_FILEPATH": "src/img/netwitness-logo.png",
        "COLLECTOR_LOG_LEVEL": "debug",
    }
...
def _then_collector_created_successfully(capfd, mock_env, collector, data) -> None:  # type: ignore
    ...
    assert daemon_config.get("netwitness_base_url") == data.get(  # noqa: S101
        "NETWITNESS_BASE_URL"
    )
    assert daemon_config.get("netwitness_username") == data.get(  # noqa: S101
        "NETWITNESS_USERNAME"
    )
    assert daemon_config.get("netwitness_password") == data.get(  # noqa: S101
        "NETWITNESS_PASSWORD"
    )
    assert daemon_config.get("netwitness_max_results") is not None  # noqa: S101
    ...
    if daemon_config.get("collector_log_level") in ["info", "debug"]:
        registered_message = "NetWitness Collector initialized successfully"
