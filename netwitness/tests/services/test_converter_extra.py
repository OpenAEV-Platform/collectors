"""Additional branch-coverage tests for the NetWitness Converter."""

import pytest
from src.services.converter import Converter
from src.services.exception import NetWitnessValidationError
from src.services.models import NetWitnessAlert
...
class TestConverterExtra:
    ...
    def test_convert_alert_with_parent_process(self):
        ...
        converter = Converter()
        alert = NetWitnessAlert(time="t", src_ip="1.2.3.4", url_path=URL_PATH)
    ...
    def test_extract_parent_process_name_no_url(self):
        ...
        converter = Converter()
        alert = NetWitnessAlert(time="t", src_ip="1.2.3.4")
    ...
    def test_extract_parent_process_name_non_matching_url(self):
        ...
        converter = Converter()
        alert = NetWitnessAlert(time="t", src_ip="1.2.3.4", url_path="/api/other")
        assert converter._extract_parent_process_name(alert) == ""  # noqa: S101

    def test_alert_data_rejects_invalid_type(self):
        """_alert_data rejects non-NetWitnessAlert input."""
        converter = Converter()
