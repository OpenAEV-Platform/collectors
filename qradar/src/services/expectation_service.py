"""IBM QRadar Expectation Service Provider.

This module contains all the IBM QRadar-specific logic for handling expectations.
...
class QRadarExpectationService:
    """IBM QRadar-specific service provider for expectation handling.

    This class contains all the business logic specific to IBM QRadar:
    - Which signature types to support (only IPV4/6 addresses)
    - How to fetch data from IBM QRadar
    ...
    def __init__(self, config: ConfigLoader | None = None) -> None:
        """Initialize the IBM QRadar service provider.
        ...
        Raises:
            ...
            QRadarConfigurationError: If service components initialization fails.
        ...
        try:
            self.logger.debug(
                f"{LOG_PREFIX} Initializing IBM QRadar service components..."
            )
            ...
            raise QRadarConfigurationError(
                f"Failed to initialize IBM QRadar service components: {e}"
            ) from e

        if (
            hasattr(config, "qradar")
            and hasattr(config.qradar, "time_window")
            and config.qradar.time_window
        ):
            self.time_window = config.qradar.time_window
            self.logger.debug(
        ...
        if hasattr(config, "qradar"):
            self.max_retry = getattr(config.qradar, "max_retry", 3)
            self.offset = getattr(
                config.qradar, "offset", timedelta(seconds=30)
            ).total_seconds()
            self.logger.debug(
    ...
    def handle_batch_expectations(
        ...
        Raises:
            QRadarExpectationError: If batch processing fails.
        ...
        try:
            ...
            for i, expectation in enumerate(expectations, 1):
                ...
                try:
                ...
                except QRadarServiceError as e:
                    self.logger.warning(
                        f"{LOG_PREFIX} IBM QRadar service error for expectation {expectation_id}: {e}"
                    )
                    ...
                    result = self._create_error_result_object(
                        QRadarExpectationError(f"Unexpected error: {e}"),
                        expectation,
                    )
    ...
    def process_expectation(
        ...
        Args:
            expectation: The expectation to process (Detection only for IBM QRadar).
        ...
        Raises:
            QRadarExpectationError: If expectation type is unsupported.
        ...
        if isinstance(expectation, DetectionExpectation):
            ...
            self.logger.warning(
                f"{LOG_PREFIX} IBM QRadar service warning for expectation {expectation_id}: IBM QRadar only supports DetectionExpectations, not PreventionExpectations, marking them as invalid"
            )
            return ExpectationResult(
                expectation_id=expectation_id,
                is_valid=False,
                expectation=expectation,
                error_message="IBM QRadar only supports DetectionExpectations, not PreventionExpectations",
            )
        else:
            self.logger.error(
    ...
    def handle_prevention_expectation(
        ...
        Since IBM QRadar only supports detection, this method logs a warning
        ...
        expectation_id = str(expectation.inject_expectation_id)
        self.logger.warning(
            f"{LOG_PREFIX} IBM QRadar service error for expectation {expectation_id}: IBM QRadar only supports DetectionExpectations, not PreventionExpectations"
        )
        return ExpectationResult(
            expectation_id=expectation_id,
            is_valid=False,
    ...
    def _handle_expectation(
        ...
        Raises:
            QRadarExpectationError: If expectation processing fails.
        ...
        try:
            ...
            self.logger.debug(
                f"{LOG_PREFIX} Fetching IBM QRadar data for {expectation_type} expectation..."
            )
            qradar_data = self.client_api.fetch_with_retry(
                search_signatures, expectation_type, self.max_retry, int(self.offset)
            )
            self.logger.debug(
                f"{LOG_PREFIX} Fetched {len(qradar_data)} data items from IBM QRadar"
            )

            self.logger.debug(
                f"{LOG_PREFIX} Converting IBM QRadar data to OAEV format..."
            )
            oaev_data = self.converter.convert_data_to_oaev_data(qradar_data)
            self.logger.debug(
    ...
    def _extract_signatures(
        ...
        Raises:
            QRadarExpectationError: If signature extraction fails.
    ...
    def _match(
        ...
        Raises:
            QRadarNoAlertsFoundError: If no data available for matching.
            QRadarNoMatchingAlertsError: If no matching alerts found.
            QRadarMatchingError: If matching process fails.

        """
        try:
            if not oaev_data:
                self.logger.debug(f"{LOG_PREFIX} No OAEV data available for matching")
                raise QRadarNoAlertsFoundError("No data available for matching")
            ...
            raise QRadarNoMatchingAlertsError()
    ...
    def _create_error_result(
        self,
        error: QRadarServiceError,
        expectation: DetectionExpectation | None = None,
    ) -> dict[str, Any]:
        """Create an error result dictionary from a IBM QRadar service error.

        Args:
            error: The IBM QRadar service error that occurred.
    ...
    def _create_error_result_object(
        ...
        """Create an ExpectationResult object from a IBM QRadar service error.
    ...
    def get_service_info(self) -> dict[str, Any]:
        ...
        """
        info = {
            "service_name": "IBM QRadar",
            "supported_signatures": [sig.value for sig in self.SUPPORTED_SIGNATURES],
            "supports_detection": True,
            "supports_prevention": False,
            "description": f"IBM QRadar expectation validation service ({len(self.SUPPORTED_SIGNATURES)} signature types, detection only)",
        }
