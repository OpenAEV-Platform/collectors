...
from src.services.utils import QRadarConfig
...
class Collector(CollectorDaemon):  # type: ignore[misc]
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            self.logger.info(  # type: ignore[has-type]
                f"{LOG_PREFIX} IBM QRadar Collector initialized successfully"
            )
    ...
    def _setup(self) -> None:
        ...
        Initializes IBM QRadar services, expectation handler, expectation manager,
        ...
        try:
            ...
            self.logger.debug(f"{LOG_PREFIX} Initializing IBM QRadar services...")

            self.qradar_service = QRadarExpectationService(self.config_instance)

            self.trace_service = QRadarTraceService(self.config_instance)

            self.expectation_handler = GenericExpectationHandler(self.qradar_service)
            ...
            service_info = self.qradar_service.get_service_info()
    ...
    def _process_callback(self) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"{LOG_PREFIX} Processing expectations using QRadar services"
            )
