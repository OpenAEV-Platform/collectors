...
class ConfigLoaderCollector(_ConfigLoaderCollector):
    ...
    for the QRadar collector instance.
    """

    id: str = Field(
        alias="Collector_ID",
        default="qradar--0b13e3f7-5c9e-46f5-acc4-33032e9b4921",
...
class ConfigLoader(ConfigBaseSettings):
    ...
    Main configuration class that combines OpenAEV, collector, and QRadar
    ...
    qradar: _ConfigLoaderQRadar = Field(
        default_factory=_ConfigLoaderQRadar,
        description="QRadar configurations.",
    )
    ...
    def to_daemon_config(self) -> Configuration:
        ...
        return Configuration(
            config_hints={
                ...
                # QRadar configuration (flattened)
                "qradar_base_url": {"data": str(self.qradar.base_url)},
                "qradar_token": {
                    "data": (
                        self.qradar.token.get_secret_value()
                        if self.qradar.token
                        else None
                    )
                },
                "qradar_username": {"data": self.qradar.username},
                "qradar_password": {
                    "data": (
                        self.qradar.password.get_secret_value()
                        if self.qradar.password
                        else None
                    )
                },
                "qradar_api_version": {"data": self.qradar.api_version},
                "qradar_data_source": {"data": self.qradar.data_source},
                "qradar_console_url": {"data": self.qradar.console_url},
