...
2. **Configure the Collector**:
    - Copy `tanium_threat_response/config.yml.sample` to `tanium_threat_response/config.yml`
    - Update configuration values or set environment variables

3. **Run the Collector**:
   ```bash
   # Using Poetry
   poetry run python -m tanium_threat_response
   
   # Or 
   poetry run TaniumThreatResponseCollector

   # Or direct execution after installing the project and activating the virtual environment:
   TaniumThreatResponseCollector
   ```
