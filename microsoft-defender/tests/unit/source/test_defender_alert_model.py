"""Unit tests for DefenderAlert Pydantic model.
...
from src.source.defender_alert_model import (
...
class TestDefenderAlert(unittest.TestCase):
    """Test DefenderAlert Pydantic model."""
    ...
    def test_alert_validates_minimal(self) -> None:
        ...
        alert = DefenderAlert.model_validate(self._minimal_alert())
    ...
    def test_alert_validates_full_with_evidence(self) -> None:
        ...
        alert = DefenderAlert.model_validate(self._alert_with_mixed_evidence())
        self.assertEqual(alert.id, "ALT-001")
...
class TestSourceDataRewrite(unittest.TestCase):
    """Test MicrosoftDefenderSourceData accepts alert dict."""

    def test_source_data_accepts_raw_alert(self) -> None:
        """Then MicrosoftDefenderSourceData accepts alert parameter."""
        from src.source.source_data import MicrosoftDefenderSourceData

        raw = {"id": "ALT-001", "title": "Test alert"}
        data = MicrosoftDefenderSourceData(alert=raw)
        self.assertEqual(data.alert, raw)

    def test_source_data_backward_compat(self) -> None:
        """Then MicrosoftDefenderSourceData still works with no args."""
        from src.source.source_data import MicrosoftDefenderSourceData

        data = MicrosoftDefenderSourceData()
