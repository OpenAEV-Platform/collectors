"""Flow-level tests for ElasticExpectationService (real processing paths)."""
...
from src.services.exception import ElasticNoAlertsFoundError
...
def _service() -> ElasticExpectationService:
    """Build an ElasticExpectationService from a test config."""
    return ElasticExpectationService(config=create_test_config())
...
class TestExpectationServiceFlow:
    ...
    def test_process_expectation_no_alerts_raises(self):
        """No fetched alerts raises ElasticNoAlertsFoundError."""
        service = _service()
    ...
    def test_create_error_result_dict(self):
        ...
        service = _service()
        error = ElasticNoAlertsFoundError("none found")

        result = service._create_error_result(error)

        assert result["is_valid"] is False  # noqa: S101
        assert result["error_type"] == "ElasticNoAlertsFoundError"  # noqa: S101
