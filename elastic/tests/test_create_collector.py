"""Test module for the Elastic Security Collector initialization."""
...
def collector_config() -> dict[str, str]:  # type: ignore
    ...
    return {
        ...
        "COLLECTOR_NAME": "Elastic Security",
        "ELASTIC_BASE_URL": "https://fake-elastic.net:9200/",
        "ELASTIC_USERNAME": "fake-user",
        "ELASTIC_PASSWORD": "fake-password",
        "ELASTIC_ALERTS_INDEX": ".alerts-security.alerts-*",
        "COLLECTOR_ICON_FILEPATH": "src/img/elastic-logo.png",
...
def test_success_create_collector(capfd, collector_config):  # type: ignore
    """Test that the main function initializes and start the Elastic Security Collector.
    ...
    """
    # Given I have a valid configuration to start the Elastic Security Collector.
...
def test_collector_config_missing_required_values() -> None:
    ...
    configuration values are missing, specifically the Elastic Security username.

    """
    # Given configuration with missing required Elastic Security username
    data = {
        "OPENAEV_URL": "http://fake-url",
        "OPENAEV_TOKEN": "fake-oaev-token",
        "OPENAEV_TENANT_ID": "deadbeef-dead-beef-dead-beefdeadbeef",
        ...
        "COLLECTOR_NAME": "Elastic Security",
        "ELASTIC_BASE_URL": "https://fake-elastic.net:9200/",
        # Missing ELASTIC_USERNAME - this should cause validation error
        "ELASTIC_PASSWORD": "fake-password",
        "ELASTIC_ALERTS_INDEX": ".alerts-security.alerts-*",
        "COLLECTOR_ICON_FILEPATH": "src/img/elastic-logo.png",
        "COLLECTOR_LOG_LEVEL": "debug",
    ...
    if "ELASTIC_USERNAME" in os_environ:
        del os_environ["ELASTIC_USERNAME"]
...
# Scenario: Create a collector with missing password
def test_collector_config_missing_password() -> None:
    """Test for the collector with missing password configuration.

    Verifies that collector creation fails appropriately when required
    password configuration is missing.

    """
    # Given configuration with missing required Elastic Security password
    data = {
        "OPENAEV_URL": "http://fake-url",
        "OPENAEV_TOKEN": "fake-oaev-token",
        "OPENAEV_TENANT_ID": "deadbeef-dead-beef-dead-beefdeadbeef",
        ...
        "ELASTIC_BASE_URL": "https://fake-elastic.net:9200/",
        "ELASTIC_USERNAME": "fake-user",
        # Missing ELASTIC_PASSWORD - this should cause validation error
        "ELASTIC_ALERTS_INDEX": ".alerts-security.alerts-*",
        "COLLECTOR_ICON_FILEPATH": "src/img/elastic-logo.png",
        "COLLECTOR_LOG_LEVEL": "debug",
    }
    ...
    if "ELASTIC_PASSWORD" in os_environ:
        del os_environ["ELASTIC_PASSWORD"]
...
def _then_collector_created_successfully(capfd, mock_env, collector, data) -> None:  # type: ignore
    ...
    assert daemon_config.get("elastic_base_url") == data.get(  # noqa: S101
        "ELASTIC_BASE_URL"
    )
    assert daemon_config.get("elastic_username") == data.get(  # noqa: S101
        "ELASTIC_USERNAME"
    )
    assert daemon_config.get("elastic_password") == data.get(  # noqa: S101
        "ELASTIC_PASSWORD"
    )
    assert daemon_config.get("elastic_alerts_index") == data.get(  # noqa: S101
    ...
    if daemon_config.get("collector_log_level") in ["info", "debug"]:
        registered_message = "Elastic Security Collector initialized successfully"
