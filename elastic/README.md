...
| Kibana URL   | elastic.kibana_url   | `ELASTIC_KIBANA_URL`        |                             | No        | Kibana base URL used to build trace links. When unset, base_url is reused with its port rewritten to 5601; set it when Kibana is elsewhere (reverse proxy, or no port in base_url). |
