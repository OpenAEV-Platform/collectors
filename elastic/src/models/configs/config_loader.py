...
class ConfigLoaderCollector(_ConfigLoaderCollector):
    ...
    for the Elastic collector instance.
    """

    id: str = Field(
        alias="Collector_ID",
        default="elastic--0b13e3f7-5c9e-46f5-acc4-33032e9b4921",
...
class ConfigLoader(ConfigBaseSettings):
    ...
    Main configuration class that combines OpenAEV, collector, and Elastic
    ...
    elastic: _ConfigLoaderElastic = Field(
        default_factory=_ConfigLoaderElastic,
            ...
            config_hints={
                ...
                "elastic_api_key": {
                    "data": (
                        ...
                        if self.elastic.api_key
                        else None
                    )
                },
                "elastic_username": {"data": self.elastic.username},
                "elastic_password": {
                    "data": (
                        self.elastic.password.get_secret_value()
                        if self.elastic.password
                        else None
                    )
                },
