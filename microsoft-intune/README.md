...
- Copy `microsoft_intune/config.yml.sample` to `microsoft_intune/config.yml`
- Update configuration values or set environment variables

3. **Run the Collector**:
   ```bash
   # Using Poetry
   poetry run python -m microsoft_intune
   
   # Or 
   poetry run MicrosoftIntuneCollector

   # Or direct execution after installing the project and activating the virtual environment:
   MicrosoftIntuneCollector
   ```
