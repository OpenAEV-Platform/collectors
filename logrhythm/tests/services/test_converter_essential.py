"""Essential tests for LogRhythm Converter service."""

from src.services.converter import Converter
from tests.services.fixtures.factories import (
    LogRhythmAlertFactory,
    create_test_logrhythm_alerts,
)


class TestConverterEssential:
    """Essential test cases for LogRhythm Converter.

    Tests the core functionality of the LogRhythm data converter including
    ...
    def test_convert_alert_with_source_and_target_ips(self):
        ...
        Verifies that LogRhythm alerts containing both source and target IPs
        are properly converted to OAEV format with correct structure.
        """
        converter = Converter()
        alert = LogRhythmAlertFactory.build(src_ip="192.168.1.100", dst_ip="10.0.0.50")
    ...
    def test_convert_multiple_alerts_list(self):
        ...
        Verifies that lists containing multiple LogRhythmAlert objects
        are processed correctly, with each alert converted to its
        appropriate OAEV format.
        """
        converter = Converter()

        alerts = create_test_logrhythm_alerts(count=3)
    ...
    def test_alert_data_type_detection(self):
        ...
        LogRhythmAlert instances vs other data types.
        """
        converter = Converter()
        alert = LogRhythmAlertFactory.build()
