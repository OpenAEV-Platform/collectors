...
from src.services.utils import LogRhythmConfig
...
COLLECTOR_TYPE = "openaev_logrhythm"
...
class Collector(CollectorDaemon):  # type: ignore[misc]
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            self.logger.info(  # type: ignore[has-type]
                f"{LOG_PREFIX} LogRhythm Collector initialized successfully"
            )
    ...
    def _setup(self) -> None:
        ...
        Initializes LogRhythm services, expectation handler, expectation manager,
        ...
        try:
            ...
            self.logger.debug(f"{LOG_PREFIX} Initializing LogRhythm services...")

            self.logrhythm_service = LogRhythmExpectationService(self.config_instance)

            self.trace_service = LogRhythmTraceService(self.config_instance)

            self.expectation_handler = GenericExpectationHandler(self.logrhythm_service)
            ...
            supported_signatures = self.logrhythm_service.get_supported_signatures()
            ...
            service_info = self.logrhythm_service.get_service_info()
    ...
    def _process_callback(self) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"{LOG_PREFIX} Processing expectations using LogRhythm services"
            )
