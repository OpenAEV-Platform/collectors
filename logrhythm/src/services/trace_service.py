"""LogRhythm Trace Service Provider.

This module provides LogRhythm-specific logic for creating expectation traces
...
from .exception import LogRhythmDataConversionError, LogRhythmValidationError

LOG_PREFIX = "[LogRhythmTraceService]"


class LogRhythmTraceService:
    """LogRhythm-specific trace service provider.
    ...
    def __init__(self, config: ConfigLoader | None = None) -> None:
        """Initialize the LogRhythm trace service.
        ...
        """
        if config is None:
            raise LogRhythmValidationError("Config is required for trace service")

        self.logger = logging.getLogger(__name__)
        self.config = config
        self.client_api = LogRhythmClientAPI(config)
        self.logger.debug(f"{LOG_PREFIX} LogRhythm trace service initialized")
    ...
    def create_traces_from_results(
        ...
        Raises:
            ...
            LogRhythmDataConversionError: If trace creation fails.

        """
        if not collector_id:
            raise LogRhythmValidationError("collector_id cannot be empty")

        if not isinstance(results, list):
            raise LogRhythmValidationError("results must be a list")
        ...
        try:
            ...
            for i, result in enumerate(valid_results, 1):
                ...
                try:
                    ...
                    raise LogRhythmDataConversionError(
        ...
        except LogRhythmDataConversionError:
            raise
        except Exception as e:
            raise LogRhythmDataConversionError(
    ...
    def _create_expectation_trace(
        ...
        """
        if not expectation_id:
            raise LogRhythmValidationError("expectation_id cannot be empty")
    ...
    def _determine_alert_name(self, matching_data: dict[str, Any]) -> str:
        ...
        """
        self.logger.debug(f"{LOG_PREFIX} Creating trace for LogRhythm alert")
        self.logger.debug(
        ...
        if (
            ...
            return "LogRhythm Detection Alert - Source IP"
        elif (
            ...
            return "LogRhythm Detection Alert - Target IP"
        else:
            self.logger.debug(
                f"{LOG_PREFIX} Using generic alert name - no specific IP data type identified"
            )
            return "LogRhythm Detection Alert"
    ...
    def _build_trace_url_from_expectation(
        ...
        """
        try:
            if not hasattr(self.config, "logrhythm"):
                self.logger.warning(
                    f"{LOG_PREFIX} No LogRhythm config available, returning empty URL"
                )
                return ""

            console_url = getattr(self.config.logrhythm, "console_url", None)
            if console_url:
                web_base_url = str(console_url).rstrip("/")
            else:
                web_base_url = str(self.config.logrhythm.base_url).rstrip("/")
            self.logger.debug(
                f"{LOG_PREFIX} Using LogRhythm console URL: {web_base_url}"
            )
            ...
            url = f"{web_base_url}/?search={query_hint}"

            self.logger.debug(f"{LOG_PREFIX} Built trace URL: {url}")
            return url

        except Exception as e:
            raise LogRhythmDataConversionError(f"Error building trace URL: {e}") from e
    ...
    def get_service_info(self) -> dict[str, Any]:
        ...
        """
        info = {
            "service_type": "logrhythm_trace",
            "supported_result_types": ["LogRhythm processing results"],
            "creates_detection_traces": True,
            "creates_prevention_traces": False,
            "description": "Creates traces from LogRhythm expectation processing results",
        }
