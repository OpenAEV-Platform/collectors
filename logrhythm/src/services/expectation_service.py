"""LogRhythm Expectation Service Provider.

This module contains all the LogRhythm-specific logic for handling expectations.
...
LOG_PREFIX = "[LogRhythmExpectationService]"


class LogRhythmExpectationService:
    """LogRhythm-specific service provider for expectation handling.

    This class contains all the business logic specific to LogRhythm:
    - Which signature types to support (only IPV4/6 addresses)
    - How to fetch data from LogRhythm
    ...
    def __init__(self, config: ConfigLoader | None = None) -> None:
        """Initialize the LogRhythm service provider.

        Args:
            config: Configuration loader instance for service settings.

        Raises:
            LogRhythmValidationError: If config is None.
            LogRhythmConfigurationError: If service components initialization fails.

        """
        if config is None:
            raise LogRhythmValidationError("Config is required for expectation service")
        ...
        try:
            self.logger.debug(
                f"{LOG_PREFIX} Initializing LogRhythm service components..."
            )
            self.client_api = LogRhythmClientAPI(config)
            ...
            raise LogRhythmConfigurationError(
                f"Failed to initialize LogRhythm service components: {e}"
            ) from e

        if (
            hasattr(config, "logrhythm")
            and hasattr(config.logrhythm, "time_window")
            and config.logrhythm.time_window
        ):
            self.time_window = config.logrhythm.time_window
            self.logger.debug(
        ...
        if hasattr(config, "logrhythm"):
            self.max_retry = getattr(config.logrhythm, "max_retry", 3)
            self.offset = getattr(
                config.logrhythm, "offset", timedelta(seconds=30)
            ).total_seconds()
            self.logger.debug(
    ...
    def handle_batch_expectations(
        ...
        Raises:
            LogRhythmExpectationError: If batch processing fails.
        ...
        try:
            ...
            for i, expectation in enumerate(expectations, 1):
                ...
                try:
                ...
                except LogRhythmServiceError as e:
                    self.logger.warning(
                        f"{LOG_PREFIX} LogRhythm service error for expectation {expectation_id}: {e}"
                    )
                    ...
                    result = self._create_error_result_object(
                        LogRhythmExpectationError(f"Unexpected error: {e}"),
                        expectation,
                    )
            ...
            raise LogRhythmExpectationError(
    ...
    def process_expectation(
        ...
        Args:
            expectation: The expectation to process (Detection only for LogRhythm).
        ...
        Raises:
            LogRhythmExpectationError: If expectation type is unsupported.
        ...
        if isinstance(expectation, DetectionExpectation):
            ...
            self.logger.warning(
                f"{LOG_PREFIX} LogRhythm service warning for expectation {expectation_id}: LogRhythm only supports DetectionExpectations, not PreventionExpectations, marking them as invalid"
            )
            return ExpectationResult(
                expectation_id=expectation_id,
                is_valid=False,
                expectation=expectation,
                error_message="LogRhythm only supports DetectionExpectations, not PreventionExpectations",
            )
        else:
            self.logger.error(
    ...
    def handle_prevention_expectation(
        ...
        Since LogRhythm only supports detection, this method logs a warning
        ...
        expectation_id = str(expectation.inject_expectation_id)
        self.logger.warning(
            f"{LOG_PREFIX} LogRhythm service error for expectation {expectation_id}: LogRhythm only supports DetectionExpectations, not PreventionExpectations"
        )
        return ExpectationResult(
            expectation_id=expectation_id,
            is_valid=False,
    ...
    def _handle_expectation(
        ...
        Raises:
            LogRhythmExpectationError: If expectation processing fails.
        ...
        try:
            ...
            self.logger.debug(
                f"{LOG_PREFIX} Fetching LogRhythm data for {expectation_type} expectation..."
            )
            logrhythm_data = self.client_api.fetch_with_retry(
                search_signatures, expectation_type, self.max_retry, int(self.offset)
            )
            self.logger.debug(
                f"{LOG_PREFIX} Fetched {len(logrhythm_data)} data items from LogRhythm"
            )

            self.logger.debug(
                f"{LOG_PREFIX} Converting LogRhythm data to OAEV format..."
            )
            oaev_data = self.converter.convert_data_to_oaev_data(logrhythm_data)
            self.logger.debug(
    ...
    def _extract_signatures(
        ...
        Raises:
            LogRhythmExpectationError: If signature extraction fails.
    ...
    def _match(
        ...
        Raises:
            LogRhythmNoAlertsFoundError: If no data available for matching.
            LogRhythmNoMatchingAlertsError: If no matching alerts found.
            LogRhythmMatchingError: If matching process fails.

        """
        try:
            if not oaev_data:
                self.logger.debug(f"{LOG_PREFIX} No OAEV data available for matching")
                raise LogRhythmNoAlertsFoundError("No data available for matching")
            ...
            raise LogRhythmNoMatchingAlertsError()
            ...
            raise LogRhythmMatchingError() from e
    ...
    def _create_error_result(
        self,
        error: LogRhythmServiceError,
        expectation: DetectionExpectation | None = None,
    ) -> dict[str, Any]:
        """Create an error result dictionary from a LogRhythm service error.

        Args:
            error: The LogRhythm service error that occurred.
    ...
    def _create_error_result_object(
        ...
        """Create an ExpectationResult object from a LogRhythm service error.
    ...
    def get_service_info(self) -> dict[str, Any]:
        ...
        """
        info = {
            "service_name": "LogRhythm",
            "supported_signatures": [sig.value for sig in self.SUPPORTED_SIGNATURES],
            "supports_detection": True,
            "supports_prevention": False,
            "description": f"LogRhythm expectation validation service ({len(self.SUPPORTED_SIGNATURES)} signature types, detection only)",
        }
