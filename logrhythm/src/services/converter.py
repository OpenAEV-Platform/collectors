"""LogRhythm Data Converter.

This module provides conversion functionality for LogRhythm data types.
...
from .models import LogRhythmAlert
from .utils.parent_process_parser import ParentProcessParser

LOG_PREFIX = "[LogRhythmConverter]"


class Converter:
    """Converter for LogRhythm data to OAEV format."""
    ...
    def __init__(self) -> None:
        ...
        self.logger.debug(f"{LOG_PREFIX} LogRhythm data converter initialized")

    def convert_data_to_oaev_data(
        self,
        data: LogRhythmAlert | list[LogRhythmAlert] | None,
    ) -> list[dict[str, Any]]:
        ...
        try:
            self.logger.debug(
                f"{LOG_PREFIX} Converting {len(data)} LogRhythm alert items to OAEV format"
            )
            oaev_datas = []
            alert_count = 0
            unknown_count = 0
            ...
            for i, item in enumerate(data, 1):
                ...
                try:
                    if self._is_alert_data(item):
                        ...
                        alert_count += 1
                        self.logger.debug(
                            f"{LOG_PREFIX} Converted LogRhythm alert item {i}"
                        )
                    else:
                        unknown_count += 1
                        self.logger.warning(
            ...
            self.logger.info(
                f"{LOG_PREFIX} LogRhythm to OAEV conversion: processed {len(data)} items -> {len(oaev_datas)} results"
            )
    ...
    def _is_alert_data(self, data: Any) -> bool:
        """Check if data is LogRhythm alert data.

        Args:
            data: Data object to check.

        Returns:
            True if data is a LogRhythmAlert instance.

        """
        return isinstance(data, LogRhythmAlert)

    def _alert_data(self, alert_data: LogRhythmAlert) -> dict[str, Any]:
        """Convert LogRhythm alert data to OAEV format.

        Args:
            alert_data: LogRhythm alert data.

        Returns:
            OAEV formatted data dictionary.

        Raises:
            LogRhythmValidationError: If input type is invalid.
        ...
        try:
            ...
            if not isinstance(alert_data, LogRhythmAlert):
            ...
            self.logger.debug(
                f"{LOG_PREFIX} Converted LogRhythm alert to OAEV with {len(oaev_data)} fields"
            )
            ...
            raise LogRhythmDataConversionError(
                f"Error converting LogRhythm alert data to OAEV: {e}"
            ) from e

    def _extract_source_ips(self, alert_data: LogRhythmAlert) -> list[str]:
        """Extract source IP addresses from alert data.

        Args:
            alert_data: LogRhythmAlert object.
    ...
    def _extract_target_ips(self, alert_data: LogRhythmAlert) -> list[str]:
    ...
    def _extract_parent_process_name(self, alert_data: LogRhythmAlert) -> str:
