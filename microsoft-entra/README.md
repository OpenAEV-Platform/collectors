...
- Copy `microsoft_entra/config.yml.sample` to `microsoft_entra/config.yml`
- Update configuration values or set environment variables

3. **Run the Collector**:
   ```bash
   # Using Poetry
   poetry run python -m microsoft_entra
   
   # Or 
   poetry run MicrosoftEntraCollector

   # Or direct execution after installing the project and activating the virtual environment:
   MicrosoftEntraCollector
   ```
