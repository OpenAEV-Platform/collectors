"""Resilient uploader with bulk-then-individual fallback strategy."""
...
__all__ = ["ResilientUploader"]


class ResilientUploader:
    """Generic bulk uploader with automatic fallback to individual uploads."""
    ...
    def prepare_bulk_data(self, data: list[Any]) -> BulkData:
        """Prepare data for bulk upload using the injected function."""
        try:
    ...
    def bulk_upload_data(self, bulk_data: BulkData) -> None:
        """Attempt bulk upload, falling back to individual on failure."""
        try:
            self.logger.debug(f"{LOG_PREFIX} Attempting bulk upload...")
            self._bulk_upload(bulk_data)
            self.logger.info(
                f"{LOG_PREFIX} Successfully bulk upload "
                f"{len(bulk_data)} {self.data_name}"
            )
        except Exception as bulk_error:
            self.logger.warning(
                f"{LOG_PREFIX} Bulk upload failed, "
                f"falling back to individual updates: {bulk_error}"
            )
            try:
                success_count = 0
                error_count = 0
    ...
    def upload_data(self, data: list[Any]) -> None:
        """Prepare then upload data, with full error handling."""
        if not data:
            self.logger.debug(
                f"{LOG_PREFIX} No {self.data_name} to upload, "
                f"skipping prepare and submit"
            )
            return

        try:
            self.logger.debug(
                f"{LOG_PREFIX} Preparing bulk {self.data_name}: "
                f"{len(data)} elements to process..."
            )
            bulk_data = self.prepare_bulk_data(data)

            if not bulk_data:
                self.logger.debug(
                    f"{LOG_PREFIX} No bulk {self.data_name} "
                    f"produced from input data"
                )
                return

            self.logger.debug(
                f"{LOG_PREFIX} Attempting bulk upload of "
                f"{len(bulk_data)} {self.data_name}..."
            )
