...
from cisco_ai_defense.configuration.config_loader import ConfigLoader
...
class OpenAEVCiscoAiDefense(CollectorDaemon):
    """Validates Cisco AI Defense detection/prevention of AI adversarial injects by replaying each
    inject's attack content through the Cisco AI Defense inspection API."""

    def __init__(self, configuration: Configuration):
        super().__init__(
            configuration=configuration,
            callback=self._process_message,
            collector_type="openaev_cisco_ai_defense",
        )
        self.collector_id = self._configuration.get("collector_id")
        self.client = CiscoAiDefenseClient(
            {
                k: self._configuration.get(k)
                for k in ("cisco_base_url", "cisco_api_key", "cisco_auth_header")
            },
            self.logger,
        )
            ...
            if inject_id not in verdicts:
                ...
                if not attack:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Cisco AI Defense scan failed for inject {inject_id}: {exc}"
                        )
                        verdicts[inject_id] = None
