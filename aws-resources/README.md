...
- Copy `aws_resources/config.yml.sample` to `aws_resources/config.yml`
- Update configuration values or set environment variables

3. **Run the Collector**:
   ```bash
   # Using Poetry
   poetry run python -m aws_resources
   
   # Or 
   poetry run AwsResourcesCollector

   # Or direct execution after installing the project and activating the virtual environment:
   AwsResourcesCollector
   ```
