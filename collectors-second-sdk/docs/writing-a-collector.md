...
This guide walks you through building a new collector using `collectors-second-sdk`. By the end you'll have a fully working collector that fetches data from a vendor API, matches it against OpenAEV expectations, and reports results.

## Prerequisites

- Python 3.12+
- Access to `collectors-second-sdk` and `xtm-oaev-sdk` packages
...
from collectors_second_sdk import OAEVData, TraceData
...
from collectors_second_sdk import CustomConfig
...
from collectors_second_sdk import ConfigBaseSettings, ConfigLoaderCustom
...
from collectors_second_sdk import BaseCollector, Source
...
```python
from collectors_second_sdk import SignatureMatcher
from xtm_oaev_sdk import SignatureTypes
...
|---|---|---|
| `BaseCollector` | `collectors_second_sdk` | Daemon lifecycle, config auto-wiring, API + ID from DaemonProtocol |
| `BasicCollectorEngine` | `collectors_second_sdk` | Default engine: fetch → filter → match → result |
| `Source` | `collectors_second_sdk` | Bundles data_fetcher + source_data + signatures |
| `ConfigBaseSettings` | `collectors_second_sdk` | Top-level loader (subclass to bind custom section) |
| `ConfigLoaderCustom` | `collectors_second_sdk` | Base for tool-specific Pydantic config |
| `SignatureMatcher` | `collectors_second_sdk` | Case-insensitive value matching with base64 decode |
| `OAEVData` | `collectors_second_sdk` | Normalized detection data model |
| `TraceData` | `collectors_second_sdk` | Trace metadata model |
| `SignatureTypes` | `xtm_oaev_sdk` | Enum of all signature type identifiers |
| `DaemonProtocol` | `collectors_second_sdk` | Behavioral contract for daemon runtime (re-exported) |
