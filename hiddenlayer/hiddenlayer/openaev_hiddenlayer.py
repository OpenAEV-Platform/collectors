...
class OpenAEVHiddenLayer(CollectorDaemon):
    """Validates HiddenLayer AIDR detection/prevention of AI adversarial injects by replaying each
    inject's attack content through the HiddenLayer Interactions endpoint."""

    def __init__(self, configuration: Configuration):
        super().__init__(
            configuration=configuration,
            callback=self._process_message,
            collector_type="openaev_hiddenlayer",
        )
        self.collector_id = self._configuration.get("collector_id")
        self.client = HiddenLayerClient(
            {
            ...
            if inject_id not in verdicts:
                ...
                if not attack:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"HiddenLayer scan failed for inject {inject_id}: {exc}"
                        )
                        verdicts[inject_id] = None
