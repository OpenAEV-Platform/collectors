...
2. **Configure the Collector**:
    - Copy `prompt_security/config.yml.sample` to `prompt_security/config.yml`
    - Update configuration values or set environment variables

3. **Run the Collector**:
   ```bash
   # Using Poetry
   poetry run python -m prompt_security
   
   # Or 
   poetry run PromptSecurityCollector

   # Or direct execution after installing the project and activating the virtual environment:
   PromptSecurityCollector
   ```
